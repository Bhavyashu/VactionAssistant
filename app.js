...
//this function is build to generate automated replyed email since it follows a certain structure
async function processUnrepliedThreads(gmail) {
  ...
  if (threads.length === 0) {
    ...
    for (const thread of threads) {//for each thread in the threads Array get we need to get to the firstMessage => beggingin of the thread get that sender info and reply them you are on vaction
      const threadId = thread.id;

      if (threadIdSet.has(threadId)) {
        const fromEmail = threadAndEmail.get(threadId);
        console.log(`Already sent a reply to ${fromEmail} from thread ${threadId}. Skipping...`);
        continue;
      }

      /*Get the thread that hasn't been replied to */
      const threadResponse = await gmail.users.threads.get({ //this functions does a deepFetch of the particular thread and all contents, messages of the thread
        userId: 'me',
        id: threadId,
      });
